...
def finalize_optimization(
    ...
    job = _advance(job_dir, job, "VALIDATING", [manifest.source_hash, diff.summary])
    ...
    if not job["status_layers"]["local_validated"]:
        job = mark_status_layer(job, "local_validated", diff.summary)
        save_checkpoint(job_dir, job, {"kind": "optimization_validation", "ref": diff.summary, "status": "verified", "at": job["updated_at"]})
        job = load_job(job_dir)
    job = _advance(job_dir, job, "CANDIDATE_READY", [diff.summary])
